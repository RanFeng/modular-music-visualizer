...
class MMVShaderMPV:
    ...
    def reset(self, depth = LOG_NO_DEPTH):
        ...
        if (self.mmv_shader_main.utils.os == "windows"):
            mpv_binary += ".exe"
        
        # Try finding mpv binary
        logging.info(f"{depth}{debug_prefix} Getting mpv binary [{mpv_binary}] path..")
        mpv_binary_path = self.mmv_shader_main.utils.get_executable_with_name(mpv_binary)
    ...
    def __generate_command(self, depth = LOG_NO_DEPTH):
        ...
        if self.output_video is not None:
            ...
            if self.mmv_shader_main.os in ["windows", "macos"]:
    ...
    def add_shader(self, shader_path, depth = LOG_NO_DEPTH):
        ...
        shader_path = self.mmv_shader_main.utils.get_abspath(shader_path)
    ...
    def input_output(self, input_video, output_video = None, depth = LOG_NO_DEPTH):
        ...
        if input_video == "last_rendered":
            ...
            last_session_info = self.mmv_shader_main.utils.load_toml(
                path = self.mmv_shader_main.interface.top_level_interace.last_session_info_file, depth = ndepth
            )
            ...
            input_video = self.mmv_shader_main.utils.get_abspath(input_video)
    ...
    def run(self, execute = True, depth = LOG_NO_DEPTH):
        ...
        if self.mmv_shader_main.prelude["flow"]["stop_at_run_command"]:
